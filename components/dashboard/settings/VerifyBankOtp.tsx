...
const VerifyBankOtp = () => {
  ...
  const handleResend = () => {
    ...
    setIsResendAvailable(false);
    setOtpValue("");
    console.log("Resend account OTP requested");
    // TODO: call resend OTP API for account verification
  };

  const handleSubmit = () => {
    if (disableBtn) return;

    console.log("Verify account OTP:", otpValue);
    setIsAlertOpen(true);
    // TODO: call account OTP verification API
  };
    ...
    <PageTransitionWrapper>
      <div className='flex flex-col gap-8 flex-1 overflow-y-auto font-poppins pb-10'>
        <form onSubmit={(e) => submitHandler(e)} className='space-y-6'>
          ...
          <p className='md:mb-12 mb-3 mt-6 text-sm text-gray-950 bg-neutral-50 px-4 py-2 rounded-[100px]'>
            {`Didn't get an OTP?`}{" "}
            <button
              type='button'
      ...
      <SuccessModal
        ...
        description='Your bank account has been successfully linked and verified. You can now use it for transactions.'
      />
    </PageTransitionWrapper>
  );
};

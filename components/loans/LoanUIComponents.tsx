...
export const SelectionCard = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`relative w-full p-5 rounded-2xl border-2 transition-all text-left flex items-center justify-between gap-4 ${selected
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      ...
      <div className="relative">
        ...
        <input
          {...props}
          className={`w-full rounded-2xl h-[56px] px-4 shadow-sm border transition-all outline-none font-medium text-[16px] ${prefix ? "pl-10" : ""
            } ${icon ? "pr-12" : ""} ${error
  ...
  return (
    <>
      ...
      {onPrev && (
        <button
          onClick={onPrev}
          className="flex-1 border bg-primary_one_600 border-gray-300 text-[#FFFFFF] rounded-full h-13 text-[16px] font-bold shadow-sm transition-colors hover:bg-gray-5"
        >
          {prevLabel}
        </button>
      )}
      <button
        ...
        className={`flex-1  hover:bg-primary_one_700 text-white rounded-full h-13 text-[16px] font-bold shadow-sm transition-all ${
      ...
      <div className="flex flex-col gap-2 w-full mt-8">
        <CustomButton title={nextLabel} onClick={onNext} disabled={isNextDisabled}  className="disabled:bg-neutral-50"/>
        <CustomButton title={prevLabel} onClick={onPrev} variant="outline" className="bg-neutral-50 " />
      </div>
    </>
...
export const ProviderCard = ({ name, logo, selected, onClick }: ProviderCardProps) => {
  return (
    <button
      onClick={onClick}
      className={`p-6 rounded-2xl border-2 transition-all flex flex-col items-center gap-4 bg-white ${selected ? "border-primary_one_600 shadow-md ring-1 ring-primary_one_600" : "border-gray-100 hover:border-gray-200"
        }`}
    >
...
export const OfferCard = ({ amount, rate, tenure, selected, onClick }: OfferCardProps) => {
  return (
    <div
      className={`p-6 rounded-3xl border-2 transition-all bg-white flex flex-col gap-4 ${selected ? "border-primary_one_600 shadow-md" : "border-gray-100"
        }`}
    >
      <div>
      ...
      <div className="flex gap-8">
        <div>
          ...
          <p className="text-[16px] font-bold text-heading">{rate}</p>
        </div>
        <div>
          <p className="text-[12px] text-gray-400 font-bold uppercase tracking-wide">Loan Tenure</p>
          <p className="text-[16px] font-bold text-heading ">{tenure}</p>
        </div>
      </div>
      <button
        onClick={onClick}
        className={`w-full h-[52px] rounded-full font-bold text-[15px] transition-all ${selected
